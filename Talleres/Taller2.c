...
void flushInput() { //   Limpia el cache del scanf (lo que puede causar loopinfinito si por ejemplo esperas un int e ingresas unchar)
    char c = '0';
    while ((c = getchar()) != '\n' && c != EOF) { }
}
...
// Funcion float que entrega el % de pacientes detectados con COVID19, respecto
// del total de pacientes, debiendo recibir como parámetros las dos variables
...
// Funcion void que imprima por pantalla el rango de temperatura alcanzado por
// los pacientes con COVID19, desplegando los siguientes datos del paciente con
// mayor y menor temperatura.
void imprimirEnPantalla(int rutMa, int edadMa, char sexoMa, float temperaturaMa, int rutMe, int edadMe, char sexoMe, float temperaturaMe, float porcentajePacientesConCovid, int hombreConCovid, int mujeresMayores40Covid, int pacientesTotales) {
...
int main() {
    ...
    int rut = 0,             // Si es 0 terminamos
        ...
        rutMa = 0, edadMa = 0, sexoMa = 0,
        temperaturaMa = 0, // Variables para el paciente con mayor temperatura
        rutMe = 0, edadMe = 0, sexoMe = 0,
        temperaturaMe = 0;   // Variables para el paciente con menor temperatura
    ...
    while ((rut = solicitarRut()) > 0) {
	    ...
	    // Si es mujer y tiene covid y es mayor de 40, sumar 1 al contador de
	    // mujeres mayores de 40 con covid
		...
		// Incremento datos de pacientes totales y pacientes covid.
	    pacientes++;
	    if (tieneCovid == 1) {
	        pacientesCovid++;
	    }
    ...
    imprimirEnPantalla(rutMa, edadMa, sexoMa, temperaturaMa, rutMe, edadMe, sexoMe, temperaturaMe, porcentajePacientes(pacientes, pacientesCovid), contadorSexoM, contadorSexoF40, pacientes);
